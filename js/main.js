...
jQuery(document).ready(function() {
	...
	jQuery('.busskexp-datepicker').datepicker({
		dateFormat: 'yy-mm-dd'
	});
	...
	jQuery('div.vert-tabs-container').each(function(){
		jQuery(this).find('div.busskex-panel:not(:first)').hide();
	});
	
	jQuery('ul.busskex-vert-tabs a').click(function(){
		var outer_panel =  jQuery(this).closest('div.vert-tabs-container');
		jQuery('ul.busskex-vert-tabs li', outer_panel).removeClass('active');
		jQuery(this).parent().addClass('active');
		jQuery('div.busskex-panel', outer_panel).hide();
	...
	$j("#order-filter-select-all-status").click(function(){
		...
		$j('.bussk-order-status input').prop('checked', true);
		return false;
	});

	$j("#order-filter-select-none-status").click(function(){
		
		$j('.bussk-order-status input').prop('checked', false);
		return false;
	});
	...
	$j('.bussk-order-status input[type=checkbox]').prop('checked', true);
